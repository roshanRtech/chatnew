import 'package:chat/screens/ShareProfile/ShareProfileScreen.dart';
...
class UserProfileScreen extends StatefulWidget {
  ...
  UserProfileScreen({required this.uid, this.heroId = '', this.receiverUser});
...
class _UserProfileScreenState extends State<UserProfileScreen>
  ...
  void init() async {
    print("-------------71>>${appSettingStore.agoraCallId}");
    isBlocked = await userService.userByEmail(getStringAsync(userEmail)).then(
        (value) => value.blockedTo!.contains(userService.ref!.doc(widget.uid)));
    if (mounted) setState(() {});
  }

  void checkUserContact() async {
    userExist = (await chatMessageService.checkUserExistContact(
  ...
  String formatTimeThis(TimeOfDay time) {
  ...
  Widget buildImageIconWidget(
    ...
    if (!currentUser.photoUrl.validate().isEmptyOrNull) {
      return Hero(
        tag: widget.uid,
        child: cachedImage(currentUser.photoUrl.validate(),
                radius: 50,
                height: 100,
                width: 100,
                fit: BoxFit.cover,
    ...
    return Container(
      height: 100,
      width: 100,
      color: getColorFromString(currentUser.uid.validate()),
      child: Text(currentUser.name.validate()[0].toUpperCase(),
              style: secondaryTextStyle(color: Colors.white))
          .center()
          .fit(),
    ).cornerRadiusWithClipRRect(100 / 2);
  }
                    ...
                    child: SingleChildScrollView(
                      child: Column(
                        children: [
                          ...
                          buildBlockMSG(),
                          buildReport(),
                          buildShareProfile(widget.receiverUser),
                        ],
                      ),
                    ),
                  ),
  ...
  String maskLast4Digits(String phoneNumber) {
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          8.height,
          Text(maskLast4Digits(currentUser.phoneNumber.validate()),
              style: secondaryTextStyle()),
          /* userExist == true
              ? Text(currentUser.phoneNumber.validate(), style: secondaryTextStyle())
              : Text('${countryCode}' + '*' * (currentUser.phoneNumber.validate().length - 3), style: secondaryTextStyle()),*/
          8.height,
          Row(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              ...
              Column(
                children: [
                  IconButton(
                    icon: Image.asset('assets/Icons/ic_call.png',
                        height: 25,
                        width: 25,
                        color:
                            appStore.isDarkMode ? Colors.white : primaryColor),
                    onPressed: () async {
                      if (await userService
                          .isUserBlocked(currentUser.uid.validate())) {
                      ...
                      return await Permissions
                          ...
                          ? CallFunctions.voiceDial(
                              context: context, from: sender, to: currentUser)
                          : {};
                    },
                  ),
                  Text('call'.translate,
              ...
              Column(
                children: [
                  IconButton(
                    icon: Image.asset('assets/Icons/ic_video_call.png',
                        height: 25,
                        width: 25,
                        color:
                            appStore.isDarkMode ? Colors.white : primaryColor),
  ...
  Widget statusWidget() {
    return Container(
      ...
      child: SettingItemWidget(
        ...
        subTitle: currentUser.updatedAt?.toDate().timeAgo.validate(),
      ),
    );
  }
  ...
  Widget buildBlockMSG() {
    return SettingItemWidget(
      ...
      title: isBlocked
          ? "${"Unblock".translate}${' ' + currentUser.name.validate()}"
          : "${"block".translate}${' ' + currentUser.name.validate()}",
  ...
  void reportBy() async {
    ...
    if (temp.length >= appSettingStore.mReportCount) {
      userService.reportUser(
          {"isActive": false}, currentUser.uid.validate()).then((value) {
        finish(context);
        finish(context);
        finish(context);
        toast(
            "${"UserAccountIsDeactivatedByAdminToRestorePleaseContactAdmin".translate}");
      ...
      userService.reportUser(
          {"reportedBy": temp}, currentUser.uid.validate()).then((value) {
        finish(context);
        finish(context);
        finish(context);
        toast(value.toString());
  ...
  Widget buildShareProfile(UserModel? receiverUser) {
    return SettingItemWidget(
      title: 'lblShareProfile'.translate,
      decoration: BoxDecoration(color: context.cardColor),
      ...
      titleTextStyle: TS.primaryTextStyle(
          color: appStore.isDarkMode ? Colors.white : Colors.black),
      onTap: () {
        _showVideoBottomSheet(context, receiverUser);
      },
    );
  }

  void _showVideoBottomSheet(BuildContext context, UserModel? receiverUser) {
        ...
        return Column(
          ...
          children: <Widget>[
            ...
            SettingItemWidget(
              ...
              onTap: () {
                String groupLink =
                    '${CHAT_WEB_DOAMAIN_URL}/${currentUser.phoneNumber?.replaceAll("+", "")}';
