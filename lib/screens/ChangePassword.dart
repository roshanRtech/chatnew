...
class _ChangePasswordState extends State<ChangePassword> {
    ...
    if (formKey.currentState!.validate()) {
      if (oldPassCont.text.toString() != getStringAsync(userPassword)) {
        ...
        await user!.updatePassword(newPassCont.text.trim()).then((value) async {
          ...
          userService
              .updateUserInfo(data, getStringAsync(userId), profileImage: null)
              .then((value) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: appBarWidget('change_password'.translate,
          showBack: true,
          elevation: 5,
          color: primaryColor,
                  ...
                  children: [
                    AppTextField(
                      ...
                      decoration: inputDecoration(context,
                          labelText: 'old_password'.translate),
                    ...
                    AppTextField(
                      ...
                      textFieldType: TextFieldType.PASSWORD,
                      decoration: inputDecoration(context,
                          labelText: 'new_password'.translate),
                      focus: newPassFocus,
                      textInputAction: TextInputAction.go,
                    ...
                    AppTextField(
                      ...
                      textInputAction: TextInputAction.go,
                      decoration: inputDecoration(context,
                          labelText: 'confirm_password'.translate),
                      focus: confPassFocus,
                      validator: (String? value) {
                        if (value!.isEmpty) return 'field_required'.translate;

                        if (value.trim().isEmpty)
                          return 'field_required'.translate;
                        if (value.trim().length < passwordLengthGlobal)
                          return 'password_length_should_be_more_than_six'
                              .translate;
                        if (value.trim() == oldPassCont.text.trim())
                          return 'old_password_should_not_be_same_as_new_password'
                              .translate;
                        return newPassCont.text == value.trim()
                            ? null
                    ...
                    AppButton(
                      ...
                      width: context.width(),
                      color: primaryColor,
                      textStyle: TS.boldTextStyle(color: white),
                    ),
                  ],
                ),
              ),
